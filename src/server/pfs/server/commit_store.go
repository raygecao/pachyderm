...
`, commit.Commit.Key(), id); err != nil {
	...
	if _, err := tx.Exec(
		`UPDATE pfs.commit SET total_filset_id = NULL WHERE int_id = $1
		`, commit.ID); err != nil {
		return errors.Wrap(err, "add file set tx")
	}
...
func getTotal(tx *pachsql.Tx, commit *pfsdb.CommitWithID) (*fileset.ID, error) {
	var id fileset.ID
	if err := tx.Get(&id,
		`SELECT total_fileset_id FROM pfs.commits
		WHERE int_id = $1
	`, commit.ID); err != nil {
...
func checkCommitDB(tx *pachsql.Tx, commit *pfsdb.CommitWithID) error {
	var id *int = new(int)
	err := tx.QueryRow(`SELECT int_id FROM pfs.commits WHERE int_id = $1`, commit.Commit.Key()).Scan(id)
	return errors.Wrap(err, "check commit id")
}
